...
module.exports = {
    authenticate(req, res, next) {
        // jwt will verfiy that we are authorized to view this route
        jwt.verify(
            ...
            (err, payload) => {
                // if we do NOT pass the check, send back a 401 unauthorized response
                if (err) {
                    res.status(401).json( {verfied: false} );
                } else {
                    // if we are valid, we will proceed to the next/callback funtion which will continue us
                    // down the process of accessing resources we are authorized to access
                    next();
                }
            }
        )
